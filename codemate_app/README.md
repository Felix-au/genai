...
This runs PyInstaller and copies the LoRA adapter alongside the exe:

```
dist/CodeMate/
├── CodeMate.exe      # Standalone executable
└── model/            # LoRA adapter files
```

> The base model is auto-downloaded on first launch (~3GB). The adapter is ~60MB.
